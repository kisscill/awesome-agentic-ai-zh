...
很多進階概念最後都會回到同一個問題：agent 的自主權到底到哪裡為止？把 agent 當成新進實習生來想：你交代一個明確的小任務、他卻自作主張把鄰近的東西也動了——這就叫「跨工作邊界」。`agent-collab-skills` 累積 dogfood 出 3 個真實案例（[完整紀錄](https://github.com/WenyuChiou/agent-collab-skills/blob/main/docs/observed-failure-modes.md)）：

- **F11（越界沒收手）**：請 codex 做字串替換 `「默認」→「預設」` 掃全專案、它順手把那份「**示範這條規則本身**」的對照表也改了。等於實習生改錯字、結果把字典裡示範那個字的範例也改掉、把規則本身的教材給汙染。
- **F12（加料）**：同樣請 codex 做字串替換、它卻擅自在多份檔案末尾加「Attribution: Karpathy / Simon Willison / ...」這些沒人要的署名。等於實習生覺得加註腳是好意、但加進別人的章節、又是沒被要求的動作 = 越權。
- **F14（知道規則但跳過）**：規則寫了「動到多語系檔案就要跑 mirror-sync 檢查」、但操作者覺得「這次很簡單、跳過」就 commit 了。結果 ship 出三個語系版本對不齊。這比 F11/F12 更難防——錯不在 agent、在操作者選擇不照 SOP 走。

**這 3 個案例告訴你的事**：

- agent 不會「剛好停在你交代的那個點」——brief 要明確寫「**只能動 X、絕對不要動 Y**」
- 規則「裝好了 ≠ 會被遵守」——必須靠機械強制（git hook / acceptance-gate preset）才擋得住人類的「這次跳過一下」

→ **實作**：工作邊界寫進 brief（用 [`agent-collab-skills`](https://github.com/WenyuChiou/agent-collab-skills) 的 task-splitter pre-task scope confirmation）+ enforce 在 acceptance gate preset + mandatory rules 加 hook（不能只是 prose）。

### 🔁 Failure-mode lifecycle（F11-F14 怎麼進化的）

![Failure-Mode Evolution Cycle](../resources/diagrams/failure-lifecycle.png)

F11/F12/F13/F14 不是隨機編號——是 [`agent-collab-skills` 累積觀察的 failure modes](https://github.com/WenyuChiou/agent-collab-skills/blob/main/docs/observed-failure-modes.md)、每個都走過 **發現 → 文件化 → encode 成 check → 自動消除** 的循環（見上圖五步）。

**F14（skipping mandatory preset）走完整循環的真實案例**：

1. **事件**：2026-05-13、Phase D 操作者跳過 preset、shipped 出 mirror drift
2. **文件化**：`observed-failure-modes.md` 命名為 F14（META-FAILURE 類別）
3. **codify 成 check**：寫進 `multi-locale-mirror-sync.yml` + `cross_document_link_text_parity` check（v0.2.3）
4. **回溯測試**：對 commit `4a60742` 跑、抓到 9+ real drift（6 個原 audit 漏的、3 個 audit 已知的）
5. **強制規則**：CLAUDE.md Complex Task Protocol 寫進「trigger 觸發時 preset 必跑」
...
1. **建立概念定位 — 四層工作邊界**：Types → Config → Repo → Service。先判斷 agent 能動到 stack 哪一層、跨層算不算違規，避免還沒挑 pattern 就先把自主權邊界搞混。
